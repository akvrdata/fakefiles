...
'''Generate CSV files with dummy data based on mimesis conifgs'''
...
class createFakeFiles:
    ...
    def extractor(self):
        '''Extracts the attributes from yaml conifg.Helps in Concurrency mapping later.TODO'''
        try:
            ...
            for table_name in self.v_dict_of_table_names:
                for cols in self.data['tables'][table_name]:
                    ...
                    list_of_rows = str(list(self.data['tables'][table_name].values())[1:]).replace("'","")
